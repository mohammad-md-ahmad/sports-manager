...
const Page = () => {
  ...
  const passwordValidation = userId ?
    string() :
  ...
  const passwordConfirmationValidation = userId ?
    string() :
    string()
  ...
  useEffect(() => {
    miscService.lists().then((response) => {
      ...
      if (userId) {
        userService.getUser(userId).then(async (response) => {

          let userData = { ...response.data.data };
          userData['createAddressRequest'] = userData['address'];

          delete userData['address'];
          setSelectCountry(userData?.createAddressRequest?.country);

          setSelectedGender({ id: userData?.gender, label: userData?.gender });
          ...
          if (userData.profile_picture) {
            userData.profile_picture = await imageUrlToBase64(userData.profile_picture);
          }
